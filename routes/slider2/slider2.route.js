const Slider = require("../../models/slider2/slider2.model");
...
const { deleteFile } = require("../../utils/image-upload");
const S3 = new AWS.S3({
...
// Get all banners
router.get("/", 
// authorizePrivilege("GET_ALL_BANNERS"),
 (req, res) => {
    Slider.find().exec().then(docs => {
        if (docs.length > 0)
...
//Add new area
router.post('/',authMiddleware, authorizePrivilege("ADD_NEW_BANNER"), upload.any(), async (req, res) => {
    if (!req.files.length) {
